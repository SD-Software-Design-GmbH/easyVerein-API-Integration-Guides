...
Alle Versionen älter als v3.0 bleiben von den Änderungen hier unberührt

## Schreibweise von Feldnamen

Alle Feldnamen sind jetzt in snake_case geändert worden und anführende Unterstriche wurden entfernt.
Das betrifft alle Endpunkte und Actions. (Actions sind Aktionen, die auf einzelnen Endpunkten ausgeführt werden können, beispielsweise `member/me/permissions` oder auch `member/{member_id}/email-notification`.)

### Im Header jeder Antwort der API:
- Das Feld `tokenRefreshNeeded`, welches impliziert, dass das Token erneuert werden muss, heißt jetzt `token_refresh_needed`.

### Authentifizierung:
- Das Feld `2FA` im `get-token` Endpunkt heißt jetzt `2_fa`.
- Das Feld `needs2FA` im `get-token` Endpunkt heißt jetzt `needs2_fa`.

### Beispiele wie Felder jetzt heißen:
- Das Feld `contactDetails` im `member` Endpunkt heißt jetzt `contact_details`.
- Das Feld `_deleteAfterDate` in allen Endpunkten heißt jetzt `delete_after_date`.
...
### Beispiele, wie Filter jetzt heißen:
- Der Filter `_chairmanPermissionGroup` im `member` Endpunkt heißt jetzt `chairman_permission_group`.
- Der Filter `customfilter` in einigen Endpunkten heißt jetzt `custom_filter`.
